...
注意体系并不等于算法，RSA 体系代指的是基于大数分解的非对称加密体系，为什么叫 RSA 体系只是因为 RSA 算法是第一个属于这种体系的算法，沿用了名称罢了
...
- ElGamal 和 D-H 是基于离散对数问题的，它们既可以在整数域也可以在****椭圆曲线****上实现
...
二次验签：对擦除后的明文和签名进行验签，和上述的验签代码一致，由于同时加入了变量 s，匹配式将保持一致
$$
