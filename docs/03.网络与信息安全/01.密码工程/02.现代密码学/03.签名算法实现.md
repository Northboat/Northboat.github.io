...
一个简单的哈希函数，将字符串映射到 G1 群上

```java
public Element h(String message){
    return HashUtil.hashStr2Group(this.getG1(), message);
}
```
...
public CryptoMap sign(String message, CryptoMap key) {
    Element sk_sig = key.get("sk_sig", this.getZr()); // 签名私钥
    Element pk_san = key.get("pk_san", this.getG1()); // 擦除公钥

    Element h  = h(message); // 明文哈希 H(m)∈G1

    // 还不确定该怎么签
    // 单用户情况下 Element sig = h.powZn(sk).getImmutable();
    Element sig = null;
...
// 使用签名者的公钥和擦除者的公钥进行验签
@Override
public Boolean verify(String message, CryptoMap key, CryptoMap signature) {
    Element pk_sig = key.get("pk_sig", this.getG1()); // g^sk
    Element pk_san = key.get("pk_san", this.getG1());

    Element sig = signature.get("sig", this.getG1()); // H(m)^sk
    Element h = h(message);

    // 还不确定该怎么验
    // 单用户情况下 return pairing(sig, g).isEqual(pairing(h, pk));
    return false;
}
```
...
在擦除时，擦除者需要用到自身的擦除私钥和签名者的签名公钥

```java
/* ========= Sanitize ========= */
@Override
public CryptoMap sanitize(String message, CryptoMap key, CryptoMap signature) {
    Element sk_san = key.get("sk_san", this.getZr());
    Element pk_sig = key.get("pk_sig", this.getG1());
    // 还不知道该怎么擦
    return signature;
}
...
二次验签：对擦除后的明文和签名进行验签，和上述的验签代码一模一样
...
public class CryptoTest {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Long> timeCost = test(sbs);
    }
