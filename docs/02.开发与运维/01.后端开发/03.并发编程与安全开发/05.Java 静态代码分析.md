...
- 其实可以用 Flex 做，很简单的事，[词法和语法分析器设计 | Arkrypto](http://arkrypto.github.io/pages/c78d5b/#词法分析器-lexer)
...
初定的想法，用户在网页上传源码文件，后端根据 API 整理分类，返回各自的调用链

- API 调用链分析，基于`javaparser-symbol-solver-core`实现
- 系统功能实现：文件上传，JSON 返回，考虑后续与大模型的对接
